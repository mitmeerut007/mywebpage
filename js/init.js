...
function dizme_tm_contact_form(){
	...
	jQuery(".contact_form #send_message").on('click', function(){
		...
		if(name===''||email===''||number===''){
			...
			jQuery.post("modal/contact.php",{ ajax_name: name, ajax_email: email, ajax_message:message, ajax_subject: subject, ajax_number: number}, function(data) {
				
				jQuery(".contact_form .returnmessage").append(data);//Append returned message to message paragraph
				
				
				if(jQuery(".contact_form .returnmessage span.contact_error").length){
					jQuery(".contact_form .invalid_mail").slideDown(500).delay(2000).slideUp(500);		
				}else{
